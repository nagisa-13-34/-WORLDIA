...
export function modal(html,{closeable=true}={}){const layer=document.querySelector('#modal-layer');layer.innerHTML=`<div class="modal">${html}</div>`;layer.hidden=false;layer.onclick=closeable?e=>{if(e.target===layer)closeModal()}:null}
export function closeModal(){const layer=document.querySelector('#modal-layer');layer.hidden=true;layer.innerHTML='';layer.onclick=null}
